...
export class DatastreamComponent implements ControlValueAccessor {
    ...
    addObservationGoal(sensorIndex, datastreamIndex, $e) {
        ...
        const goals = this.parentForm.get(
            `sensors.${sensorIndex}.datastreams.${datastreamIndex}.observationGoals`,
        ).value;
    ...
    removeObservationGoal(sensorIndex, datastreamIndex, item) {
        const observationGoals = this.parentForm.get(
            `sensors.${sensorIndex}.datastreams.${datastreamIndex}.observationGoals`,
        ).value;
    ...
    search = (text$: Observable<string>) =>
        text$.pipe(
            ...
            switchMap((x) =>
                this.observationGoalService.getObservationGoals({
                    pageIndex: 0,
                    pageSize: 15,
                    name: x,
                }),
