...
export class ObservationGoalsComponent implements OnInit, OnDestroy {
  ...
  public async getPage(pageIndex) {
    const goalsPromise = this.observationGoalService.getObservationGoals({
      pageIndex, pageSize: this.pageSize, sortField: this.sortField, sortDirection: this.sortDirection,
    }).toPromise();
