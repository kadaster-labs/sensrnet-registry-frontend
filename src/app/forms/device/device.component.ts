...
import {
  ...
  IUpdateLocationBody,
  IUpdateSensorBody,
  DeviceService, IRegisterDeviceBody
...
export class DeviceComponent implements OnInit, OnDestroy {
  ...
  public deviceFormControlSteps: Array<Array<any>>;
  ...
  public deviceRegisterInvalidMessage = $localize`:@@device.register.invalid:The form is invalid`;
  public deviceRegisterFailedMessage = $localize`:@@device.register.failure:An error has occurred during registration:`;
  public deviceRegisteredOrganizationMessage = $localize`:@@device.register.org:You need to join an organization first`;
  ...
  public setNameFromCategory(item: string) {
    this.deviceForm.controls.name.setValue(getTypeTranslation(item));
  }
  ...
  public async submitDevice() {
    ...
    if (!this.deviceForm.valid) {
      this.alertService.error(this.deviceRegisterInvalidMessage, false, 4000);
      return;
    }
  ...
  public async registerDevice() {
    ...
    try {
      await this.deviceService.register(device);

      this.locationService.showLocation(null);
      this.alertService.success(this.deviceRegisterSuccessMessage, false, 4000);
    } catch (error) {
      this.alertService.error(`${this.deviceRegisterFailedMessage}: ${error.toString()}.`);
    }
  ...
  public async submit() {
    ...
    const claim = this.connectionService.currentClaim;
    if (!claim) {
      this.alertService.error(this.deviceRegisteredOrganizationMessage);
      return;
    }
  ...
  public ngOnInit() {
    ...
    this.subscriptions.push(
      this.route.params.subscribe(async params => {
        if (params.id) {
          const device = await this.deviceService.get(params.id);
